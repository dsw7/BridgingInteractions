...
(1) Ensure both Python3 and *SQL are installed.
(2) Open the main.py script in a Python editor of choosing and assign a range of PDB files to iterate over:
    (2.1) Find START and END variables and assign the following:
              START = 0
              END = -1
          To iterate over entire PDB.
          Another example: START = 1000 and END = 2000 will iterate over 1000 files.
(3) Set Met-Aromatic algorithm parameters. The default settings are the recommended settings.
...
(4) Execute the script. Mining the entire PDB should take about a day (dependent on network bit rate).
(5) Execute counts.py to analyze data in the *SQL database once the mining job is complete.
(6) The counts.py assumes that the NetworkX library (https://networkx.github.io/) is available. 
