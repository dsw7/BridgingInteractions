...
from methods        import *                        # local in sys.path
...
def met_aromatic(filepath, CHAIN, CUTOFF=6.0, ANGLE=109.5, MODEL='cp'):
    ...
    model_first = []
    for line in data:
        if line[IDX_ATOM] != 'ENDMDL': model_first.append(line)
        else: break
    ...
    for grouped in DATA_ARO:
        ...
        for row in grouped:
            if row[3] == 'PHE':   row[2] = DICT_ATOMS_PHE.get(row[2])
            elif row[3] == 'TYR': row[2] = DICT_ATOMS_TYR.get(row[2])
            else:                 row[2] = DICT_ATOMS_TRP.get(row[2])
    ...
    # apply distance then angular conditions 
    ...
    # **************************************
    # modification for scalene triangle study
    interacting_met_positions = [i[3] for i in RESULT]
    interacting_met_positions = list(set(interacting_met_positions))
    
    # work backwards to get original data for interacting methionines
    RAW_MET_DATA = []
    for line in DATA_MET:
        if line[5] in interacting_met_positions and line[2] == 'SD':
            RAW_MET_DATA.append(line)
    # **************************************
            
            
    return RESULT, RAW_MET_DATA
