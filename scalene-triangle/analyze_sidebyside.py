...
size = 8.5   # plot size in inches
adj = 0.35  # float to pass into subplot adjust
save_plot = False
...
f, axarr = plt.subplots(1, 4, figsize=(size, 1 / 4 * size))
f.suptitle('PHE : MET : PHE', size=14, x=0.02, y=0.5, ha='center', va='center', rotation='vertical')
f.subplots_adjust(wspace=adj)
axarr[0].hist(AREAS, color='b', edgecolor='k', bins=bin_choice)
axarr[0].set_title('AREAS', size=12)
axarr[0].set_xlabel(r'Area / $\AA^2$', size=12)
axarr[1].hist(VEC_A, color='r', edgecolor='k', bins=bin_choice)
axarr[1].set_title('SF / SD', size=12)
axarr[1].set_xlabel(r'Length / $\AA$', size=12)
axarr[2].hist(VEC_B, color='r', edgecolor='k', bins=bin_choice)
axarr[2].set_title('MT / SD', size=12)
axarr[2].set_xlabel(r'Length / $\AA$', size=12)
axarr[3].hist(VEC_C, color='r', edgecolor='k', bins=bin_choice)
axarr[3].set_title('SF / MT', size=12)
axarr[3].set_xlabel(r'Length / $\AA$', size=12)

for a in range(0, 4):
    axarr[a].spines['right'].set_visible(False)
    axarr[a].spines['top'].set_visible(False)
    if a == 0:
        axarr[a].set_ylabel('Frequency', size=12)
        axarr[a].xaxis.set_major_locator(ticker.MultipleLocator(20))
    else:
        axarr[a].xaxis.set_major_locator(ticker.MultipleLocator(10))
    axarr[a].yaxis.set_major_locator(ticker.MultipleLocator(500))
    axarr[a].set_ylim(0, 3500)
    axarr[a].tick_params(axis='x', rotation=90, labelsize='small')
    
if save_plot:
    plt.savefig('all_phe.png', dpi=1000, bbox_inches='tight')
...
f, axarr = plt.subplots(1, 4, figsize=(size, 1 / 4 * size))
f.suptitle('PHE : MET : ARO', size=14, x=0.02, y=0.5, ha='center', va='center', rotation='vertical')
f.subplots_adjust(wspace=adj) 
axarr[0].hist(AREAS, color='b', edgecolor='k', bins=bin_choice)
axarr[0].set_title('AREAS', size=12)
axarr[0].set_xlabel(r'Area / $\AA^2$', size=12)
...
for a in range(0, 4):
    ...
    if a == 0:
        ...
        axarr[a].xaxis.set_major_locator(ticker.MultipleLocator(10))
    axarr[a].yaxis.set_major_locator(ticker.MultipleLocator(500))
    axarr[a].set_ylim(0, 4500)
    axarr[a].tick_params(axis='x', rotation=90, labelsize='small')
    
if save_plot:
    plt.savefig('phe_aro.png', dpi=1000, bbox_inches='tight')    
...
f, axarr = plt.subplots(1, 4, figsize=(size, 1 / 4 * size))
f.suptitle('ARO : MET : ARO', size=14, x=0.02, y=0.5, ha='center', va='center', rotation='vertical')
f.subplots_adjust(wspace=adj) 
axarr[0].hist(AREAS, color='b', edgecolor='k', bins=bin_choice)
axarr[0].set_title('AREAS', size=12)
axarr[0].set_xlabel(r'Area / $\AA^2$', size=12)
...
for a in range(0, 4):
    ...
    if a == 0:
        ...
        axarr[a].xaxis.set_major_locator(ticker.MultipleLocator(10))
    axarr[a].yaxis.set_major_locator(ticker.MultipleLocator(500))
    axarr[a].set_ylim(0, 2000)
    axarr[a].tick_params(axis='x', rotation=90, labelsize='small')

if save_plot:
    plt.savefig('aro_aro.png', dpi=1000, bbox_inches='tight')    
